...
function start(robot, d1, d2, d3, d4)::nothing
    along(robot, d1)
    along(robot, d2)
    along(robot, d1)
    along(robot, d2)
...
function result(robot)::nothing
    start(robot, Nord, West, Sud, Ost)
    flex(robot, Ost, West, Nord, Sud)
 
    start(robot, Nord, Ost, Sud, West)
    flex(robot, West, Ost, Sud, Nord)
 
    start(robot, Sud, Ost, Nord, West)
    flex(robot, Nord, Sud, Ost, West)
 
    start(robot, Sud, West, Nord, Ost)
